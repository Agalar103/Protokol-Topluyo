...
const VoiceArea: React.FC<VoiceAreaProps> = ({ channel, members, voiceState, setVoiceState, screenShare, setScreenShare }) => {
  ...
  return (
    <div className="flex-1 flex flex-col bg-[#0b0314] overflow-hidden relative">
      <div className="absolute top-0 left-0 right-0 h-1 bg-gradient-to-r from-transparent via-[#00ffff]/20 to-transparent" />
      
      {/* Top Header */}
      <div className="p-8 border-b border-white/5 flex items-center justify-between shrink-0">
         <div className="space-y-1">
            <h2 className="text-4xl font-[1000] text-white uppercase italic tracking-tighter leading-none">{channel.name}</h2>
         ...
         <div className="flex gap-4">
           ...
           {isStage && (
             <button className="bg-[#ff00ff] text-white px-8 py-3 font-[1000] uppercase italic tracking-tighter border-2 border-white shadow-2xl hover:scale-105 transition-all">
               KONUŞMA İSTEĞİ
             </button>
           )}
         </div>
      ...
      <div className="flex-1 overflow-y-auto p-12 no-scrollbar">
         {isStage ? (
            <div className="space-y-16">
               <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
                     <div className="aspect-square bg-[#110524] border-4 border-[#ff00ff] relative overflow-hidden group shadow-[0_0_30px_rgba(255,0,255,0.1)]">
                     ...
                     {members.slice(0, 3).map(m => (
                       <div key={m.id} className="aspect-square bg-[#110524] border-4 border-[#00ffff] relative overflow-hidden group shadow-[0_0_30px_rgba(0,255,255,0.1)]">
                          <img src={m.avatar} className="w-full h-full object-cover grayscale hover:grayscale-0 transition-all duration-700" alt="" />
                          <div className="absolute bottom-0 inset-x-0 p-4 bg-gradient-to-t from-black to-transparent">
               ...
               <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-4 lg:grid-cols-8 gap-4 opacity-50">
                     {members.map(m => (
                        <img key={m.id} src={m.avatar} className="aspect-square rounded-xl bg-purple-900 border-2 border-white/10" alt="" />
                     ))}
                  </div>
               </div>
            </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-8">
               {/* Music Bot Speaker */}
               <div className="aspect-video bg-[#110524] rounded-2xl border-2 border-[#ff00ff] shadow-[0_0_20px_rgba(255,0,255,0.2)] flex flex-col items-center justify-center gap-4 group relative overflow-hidden">
                  ...
                  <span className="text-[10px] font-[1000] text-[#ff00ff] uppercase tracking-widest italic">MUSIC MASTER BOT (YAYINDA)</span>
               </div>
               {members.slice(0, 11).map(m => (
                  <div key={m.id} className={`aspect-video bg-[#110524] rounded-2xl border-2 transition-all group relative overflow-hidden ${m.id === 'admin-1' && !voiceState.isMuted ? 'border-green-500 shadow-[0_0_20px_rgba(34,197,94,0.3)]' : 'border-white/5'}`}>
                     {m.id === 'admin-1' && voiceState.isVideoOn ? (
                       <div className="w-full h-full bg-black flex items-center justify-center relative">
                         ...
                         <span className="text-white/20 font-black uppercase text-4xl italic">VIDEO_FEED</span>
                       </div>
                     ) : (
                       <div className="w-full h-full flex flex-col items-center justify-center gap-4">
                          <img src={m.avatar} className="w-20 h-20 rounded-2xl grayscale" alt="" />
                          <span className="text-[10px] font-[1000] text-white/40 uppercase tracking-widest">{m.username}</span>
                       </div>
                     )}
                  </div>
               ))}
      ...
      <div className="h-32 bg-[#05010a] border-t-4 border-[#ff00ff]/10 flex items-center justify-center gap-6 relative px-8 shrink-0">
         ...
         <button 
           ...
           className={`p-6 border-2 transition-all ${voiceState.isMuted ? 'bg-red-600 border-red-400 text-white shadow-2xl' : 'bg-white/5 border-white/10 text-white hover:border-[#ff00ff]'}`}
         >
         ...
         <button 
           ...
           className={`p-6 border-2 transition-all ${voiceState.isVideoOn ? 'bg-[#00ffff] border-white text-black' : 'bg-white/5 border-white/10 text-white hover:border-[#00ffff]'}`}
         >
         ...
         <button className="px-10 py-5 bg-[#ff00ff] border-4 border-white text-white font-[1000] uppercase italic tracking-tighter text-sm hover:scale-105 transition-all shadow-2xl active:translate-y-1">EKRAN PAYLAŞ</button>

         <button className="p-6 bg-red-600/10 text-red-600 border-2 border-red-600/30 font-black italic uppercase text-xs tracking-widest hover:bg-red-600 hover:text-white transition-all">AĞI TERK ET</button>
      </div>
    </div>
  );
};
