...
const LandingPage: React.FC<LandingPageProps> = ({ onLogin }) => {
  return (
    <div className="min-h-screen bg-[#05010a] text-white selection:bg-[#ff00ff] selection:text-black scroll-smooth overflow-y-auto">
      {/* 1. Header Navigation */}
      <nav className="fixed top-0 left-0 right-0 z-50 flex items-center justify-between px-8 py-6 max-w-[1400px] mx-auto transition-all duration-300 backdrop-blur-sm bg-black/10">
        ...
        <div className="hidden lg:flex items-center gap-10 font-black text-xs uppercase tracking-widest text-white/70">
          <a href="#" className="hover:text-[#ff00ff] transition-all hover:tracking-[0.2em]">Yükle</a>
          <a href="#" className="hover:text-[#ff00ff] transition-all hover:tracking-[0.2em]">Nitro</a>
          <a href="#" className="hover:text-[#ff00ff] transition-all hover:tracking-[0.2em]">Mağaza</a>
          <a href="#" className="hover:text-[#ff00ff] transition-all hover:tracking-[0.2em]">Hakkımızda</a>
        </div>

        <button 
          onClick={onLogin}
          className="magnetic-btn bg-transparent border-2 border-[#00ffff] text-[#00ffff] px-8 py-3 rounded-none font-black text-xs uppercase tracking-[0.2em] hover:bg-[#00ffff] hover:text-black transition-all shadow-[0_0_15px_rgba(0,255,255,0.4)]"
        >
          Sisteme Gir
        </button>
      </nav>
      ...
      <section className="relative min-h-screen flex flex-col items-center justify-center pt-24 pb-12 px-6 overflow-hidden">
        ...
        <div className="max-w-[1200px] w-full flex flex-col items-center z-10 text-center">
          <div className="mb-6 bg-white/5 border border-white/10 px-4 py-2 rounded-none backdrop-blur-md animate-float">
            <span className="text-[10px] font-black tracking-[0.4em] uppercase text-[#00ffff]">Protocol v2.5.0 Active</span>
          </div>
          
          <h1 className="text-6xl md:text-[10rem] font-[1000] leading-[0.8] mb-12 tracking-tighter uppercase drop-shadow-[0_10px_30px_rgba(0,0,0,1)] flex flex-col items-center reveal-item">
            <span className="relative">
              DURMA
              <span className="absolute -top-2 -right-6 text-xl text-[#ff00ff] italic animate-bounce">punk</span>
            </span>
            <span className="text-transparent bg-clip-text bg-gradient-to-b from-white to-white/20">KONUŞ</span>
            <span className="text-[#00ffff] italic text-4xl md:text-6xl mt-4 drop-shadow-[0_0_15px_rgba(0,255,255,0.5)] animate-shiny">Sınırları Zorla</span>
          </h1>

          <p className="text-lg md:text-2xl leading-relaxed mb-16 font-medium max-w-2xl opacity-70 italic reveal-item" style={{ animationDelay: '0.2s' }}>
          ...
          <div className="flex flex-col sm:flex-row gap-6 reveal-item" style={{ animationDelay: '0.4s' }}>
            <button 
              onClick={onLogin}
              className="group relative magnetic-btn bg-[#ff00ff] text-black px-12 py-6 font-[1000] text-xl uppercase tracking-tighter hover:scale-105 transition-all shadow-xl"
            >
            ...
            <button 
              onClick={onLogin}
              className="magnetic-btn bg-white/5 border-2 border-white/10 text-white px-12 py-6 font-[1000] text-xl uppercase tracking-tighter hover:bg-white/10 transition-all hover:border-[#00ffff] shadow-lg"
            >
              Tarayıcıdan Sız
            </button>
          </div>
      ...
      <section className="bg-black py-40 px-6 relative overflow-hidden border-y border-[#ff00ff]/20">
        ...
        <div className="max-w-[1200px] mx-auto grid grid-cols-1 lg:grid-cols-2 items-center gap-32">
          <div className="relative group">
            <div className="absolute -inset-10 bg-[#ff00ff]/10 rounded-full blur-[100px] group-hover:opacity-100 opacity-50 transition-opacity" />
            <img 
              src="https://images.unsplash.com/photo-1614850523296-d8c1af93d400?q=80&w=2070&auto=format&fit=crop" 
              className="relative z-10 w-full rounded-none border-4 border-[#ff00ff] shadow-[20px_20px_0px_rgba(255,0,255,0.2)] grayscale group-hover:grayscale-0 group-hover:scale-105 transition-all duration-700" 
              alt="Community" 
            />
          </div>
          <div className="space-y-10 reveal-item">
            <h2 className="text-5xl md:text-7xl font-[1000] leading-none tracking-tighter uppercase italic text-[#00ffff] animate-shiny">
            ...
            <button className="magnetic-btn text-[#ff00ff] font-black text-sm uppercase tracking-[0.3em] hover:underline underline-offset-8">Keşfetmeye Başla →</button>
          </div>
        </div>
      </section>
        ...
        <div className="max-w-[1400px] mx-auto grid grid-cols-1 md:grid-cols-4 gap-20 mb-32">
          <div className="col-span-2">
            <div className="flex items-center gap-4 mb-8">
              <Logo />
              <span className="text-3xl font-[1000] tracking-tighter uppercase italic text-transparent bg-clip-text bg-gradient-to-r from-white to-white/40">Topluyo</span>
            </div>
          ...
          <div className="space-y-6">
            ...
            <ul className="space-y-4 text-sm font-bold text-white/40">
              <li className="hover:text-[#ff00ff] hover:translate-x-2 cursor-pointer transition-all">Destek</li>
              <li className="hover:text-[#ff00ff] hover:translate-x-2 cursor-pointer transition-all">Güvenlik</li>
              <li className="hover:text-[#ff00ff] hover:translate-x-2 cursor-pointer transition-all">Blog</li>
            </ul>
          </div>
          <div className="space-y-6">
            <h4 className="text-[#ff00ff] font-black uppercase text-xs tracking-widest">Sosyal Medya</h4>
            <ul className="space-y-4 text-sm font-bold text-white/40">
              <li className="hover:text-[#00ffff] hover:translate-x-2 cursor-pointer transition-all">Twitter</li>
              <li className="hover:text-[#00ffff] hover:translate-x-2 cursor-pointer transition-all">Instagram</li>
              <li className="hover:text-[#00ffff] hover:translate-x-2 cursor-pointer transition-all">TikTok</li>
            </ul>
          </div>
        </div>
