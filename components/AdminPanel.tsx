...
import { Member, MessageType, AuditLog } from '../types';
...
const AdminPanel: React.FC<AdminPanelProps> = ({ members, logs, activeTab, setActiveTab, onClose, currentUser, onSendSystemMessage }) => {
      ...
      <div className="relative w-full max-w-6xl bg-[#05010a] border-4 border-red-600/50 shadow-[0_0_50px_rgba(220,38,38,0.3)] overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="p-8 border-b border-red-600/20 bg-red-600/5 flex items-center justify-between shrink-0">
          <div>
            ...
            <p className="text-[10px] font-black text-red-500 uppercase tracking-[0.4em]">YÖNETİM_VE_SİBER_GÜVENLİK_KONTROLÜ v4.5</p>
          </div>
          <div className="flex gap-4">
             <div className="flex bg-black border border-white/5 p-1">
                <button 
                  onClick={() => setActiveTab('members')}
                  className={`px-6 py-2 text-[10px] font-black uppercase italic transition-all ${activeTab === 'members' ? 'bg-red-600 text-white' : 'text-white/40 hover:text-white'}`}
                >
                  ÜYELER
                </button>
                <button 
                  onClick={() => setActiveTab('logs')}
                  className={`px-6 py-2 text-[10px] font-black uppercase italic transition-all ${activeTab === 'logs' ? 'bg-red-600 text-white' : 'text-white/40 hover:text-white'}`}
                >
                  SİSTEM_LOGLARI
                </button>
             </div>
        ...
        <div className="flex-1 overflow-y-auto p-8 no-scrollbar">
          {activeTab === 'members' ? (
            ...
            <div className="space-y-4">
               <div className="bg-red-600/10 p-4 border border-red-600/20 mb-8">
                  <p className="text-[10px] font-[1000] text-red-500 uppercase italic tracking-[0.2em]">DİKKAT: BU ALAN TÜM KULLANICI AKTİVİTELERİNİ (YAZILANLAR, SİLİNENLER, PROFİL DEĞİŞİKLİKLERİ) GERÇEK ZAMANLI LİSTELER.</p>
               </div>
               
               <table className="w-full text-left">
                  <thead>
                    <tr className="border-b border-white/5 text-red-500/50 text-[10px] font-black uppercase tracking-[0.2em]">
                      <th className="pb-4 pl-4">Zaman</th>
                      <th className="pb-4">Kullanıcı</th>
                      <th className="pb-4">Eylem</th>
                      <th className="pb-4">Detay / İçerik</th>
                      <th className="pb-4 text-right pr-4">Raw_Data</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-white/5">
                    {logs.length === 0 ? (
                       <tr>
                          <td colSpan={5} className="py-20 text-center opacity-20 italic">HENÜZ BİR LOG VERİSİ SAPTANMADI...</td>
                       </tr>
                    ) : (
                      logs.map((log) => (
                        <tr key={log.id} className="hover:bg-white/[0.02] transition-colors group">
                           ...
                           <td className="py-4">
                              <div className="flex items-center gap-2">
                                 <span className="text-xs font-black uppercase italic text-white/80">{log.username}</span>
                                 <span className="text-[8px] font-mono text-white/10">{log.userId.slice(0, 8)}...</span>
                              </div>
                           </td>
                           <td className="py-4">
                              <span className={`text-[10px] font-[1000] uppercase italic tracking-widest ${getActionColor(log.action)}`}>
                                 {log.action}
                              </span>
                           </td>
                           <td className="py-4">
                              <p className="text-xs font-medium text-white/60 max-w-md truncate group-hover:whitespace-normal group-hover:overflow-visible transition-all">
                                 {log.details}
                              </p>
                           </td>
                           <td className="py-4 text-right pr-4">
                              <button 
                                onClick={() => console.log(log.rawData)}
                                className="text-[8px] font-black text-white/10 hover:text-[#00ffff] border border-white/5 hover:border-[#00ffff]/30 px-2 py-1 transition-all"
                              >
                                VIEW_JSON
                              </button>
                           </td>
        ...
        {/* Footer */}
        <div className="p-6 bg-red-600/10 border-t border-red-600/20 text-[9px] font-black text-red-500 uppercase tracking-[0.5em] text-center italic shrink-0">
